...
class SoundCloudApi(private val storage: TokenStorage) {
    ...
    private fun Request.Builder.webHeaders(token: String): Request.Builder = this
    ...
    suspend fun likeTrack(trackId: Long): Boolean = withContext(Dispatchers.IO) {
        runCatching {
            ...
            // Try 1: v2 with browser headers (token was captured from a web session)
            ...
            // Try 2: v1 with browser headers + oauth_token in URL
            req = Request.Builder()
